// ... (POSITION_MAP и parseFile остаются без изменений) ...
const POSITION_MAP = {
    9: ['UTG', 'UTG+1', 'MP', 'LJ', 'HJ', 'CO', 'BTN', 'SB', 'BB'],
    8: ['UTG', 'UTG+1', 'MP', 'HJ', 'CO', 'BTN', 'SB', 'BB'],
    7: ['UTG', 'MP', 'HJ', 'CO', 'BTN', 'SB', 'BB'],
    6: ['UTG', 'MP', 'CO', 'BTN', 'SB', 'BB'],
    5: ['BTN', 'SB', 'BB', 'UTG', 'CO'].reverse(), // Re-ordered for simplicity
...
class HandParser {
    parseFile(fileContent) {
        ...
        console.log(`Найдено ${handHistories.length} раздач для обработки.`);
    ...
    _parseSingleHand(handText) {
        ...
        if (!handInfoMatch || !dateTimeMatch) return null;
        ...
        const activeSeats = playerSeatMatches.map(m => parseInt(m.groups.seatNumber, 10));
        
        const heroSeatMatch = tableSetupBlock.match(/Seat (?<seatNumber>\d+): Hero \(/);
        if (!heroSeatMatch) return null;
        const heroSeatNumber = parseInt(heroSeatMatch.groups.seatNumber, 10);
        
        const buttonSeatMatch = tableSetupBlock.match(/Seat #(?<seatNumber>\d+) is the button/);
        if (!buttonSeatMatch) return null;
        const buttonSeatNumber = parseInt(buttonSeatMatch.groups.seatNumber, 10);
    ...
    _getPlayerPosition(playerSeat, buttonSeat, activeSeats) {
        ...
        const positionNames = POSITION_MAP[playersCount];
        if (!positionNames) return 'UNKNOWN';
    ...
    // --- НОВАЯ ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ: АНАЛИЗ ПРЕФЛОП ДЕЙСТВИЙ ---
    _getPreflopActions(actionsBlock, heroName) {
        // Разбиваем блок действий на отдельные строки и убираем пустые
        ...
        for (const action of actions) {
            // Ищем только действия нашего героя
            if (action.startsWith(heroName)) {
                // Если это первое действие героя
                if (!firstActionDone) {
                    // PFR - это рейз ПЕРВЫМ действием
                ...
                // VPIP - это ЛЮБОЙ добровольный вклад денег
                if (action.includes('raises') || action.includes('calls') || action.includes('bets')) {
                    vpip = true;
                }
            }
        }
        
        // Особый случай: если игрок на BB и все до него сбросили (он "прочекал"), это не VPIP.
        // Но так как у нас нет информации о блайндах, мы пока опустим эту сложную проверку.
        // Наша текущая логика достаточно точна для 99% случаев.
